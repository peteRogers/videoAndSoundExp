...
struct VideoAudioKitView: View {
    ...
    var body: some View {
        ...
        .onChange(of: serialModel.val0) { oldValue, newValue in
            ...
            model.manipulateAudio(sensor: Double(newValue))
            print(newValue)
...
final class VideoAudioKitModel: ObservableObject {
    ...
    private var trans: TransientShaper?
    ...
    func loadVideoFromBundle(named name: String, ext: String) async {
        ...
        // IMPORTANT: mute the AVPlayer so you only hear AudioKit's processed audio.
        vp.isMuted = true
        vp.volume = 0
        self.videoPlayer = vp
    ...
    // MARK: MANIPULATE!!!!
    func manipulateAudio(sensor:Double){
        //does panning
        let p = mapRange(value: Float(sensor), inMin: 0.0, inMax: 200, outMin: -1, outMax: 1)
        panner?.pan = p
        
        
        //does flanger
        let depth = mapRange(value: Float(sensor), inMin: 0.0, inMax: 200, outMin: 0.5, outMax: 1)
        let fb = mapRange(value: Float(sensor), inMin: 0.0, inMax: 200, outMin: 0.25, outMax: 0.75)
        flanger?.depth = depth
        flanger?.feedback = fb
        flanger?.dryWetMix = 1.0
        
        let pitch = mapRange(value: Float(sensor), inMin: 0.0, inMax: 200, outMin: 0.0, outMax: 3000)
    ...
    private func buildAudioGraphIfNeeded() throws {
        ...
        flanger = Flanger(timePitch!)
        //trans = TransientShaper(flanger!)
        //decimator = Decimator(trans!)
        //panner = Panner(decimator!)\
        panner = Panner(flanger!)
